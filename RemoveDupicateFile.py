...
def FindDuplicate(DirectoryName = "Marvellous"):  # Newly Added
    ...
    for FolderName , SubFolderNames, FileNames in os.walk(DirectoryName): #loop travel for each SubFolderNames
        for fname in FileNames:                                         
            fname = os.path.join(FolderName,fname)
            checksum = CalculateChecksum(fname)
